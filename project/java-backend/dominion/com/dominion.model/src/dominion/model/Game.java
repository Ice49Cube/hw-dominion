...
public class Game {
    ...
    public Game(Connection con, String[] playerNames, String cardSetName) throws Exception {
        ...
        System.out.println("New game cardSet: " + this.cardSet);
        this.id = this.insertGameIntoGames(con, cardSet.getName());
    ...
    private GameCard insertCardIntoGameCards(Connection con, String deck, CardInfo cardInfo, int numberOfPlayers) throws Exception {
        ...
        int id = Database.executeInsert(con, sql, new Object[]{this.id, cardInfo.getName(), deck, count, cardInfo.getCost(), cardInfo.getValue(), cardInfo.getIsAction() ? 1 : 0, cardInfo.getIsCoin() ? 1 : 0});
        GameCard card = new GameCard(id, this, cardInfo.getName(), deck, count, cardInfo.getCost(), cardInfo.getValue(), cardInfo.getIsAction(), cardInfo.getIsCoin());
        this.cards.put(card.getId(), card);
        return card;
    }

    private void insertCardsIntoPlayerCards(Connection con, Player player, boolean first, int copper, int estate) throws Exception {
    ...
    private PlayerCard insertCardIntoPlayerCards(Connection con, Player player, int cardId, String pile, int order) throws Exception {
        ...
        return new PlayerCard(id, player, cardId, pile, order);
    }

    private int insertGameIntoGames(Connection con, String cardSet) throws Exception {
        return Database.executeInsert(con, "INSERT INTO games (cardset) VALUES (?)", new Object[]{cardSet});
    }
    ...
    private void loadCards(Connection con) throws Exception {
        String sql = "SELECT * FROM gamecards"
    ...
    private void loadPlayers(Connection con) throws Exception {
        String sql = "SELECT * FROM playercards"
                ...
                + " WHERE games.id = ?"
                + " ORDER BY players.id, pile, `order`";
        Player player = null;
        try (PreparedStatement stmt = con.prepareStatement(sql);
                ResultSet rs = Database.executeQuery(con, stmt, new Object[]{this.id})) {
            while (rs.next()) {
    ...
    private Player readPlayer(ResultSet rs) throws Exception {
        return new Player(
                this,
                rs.getInt("players.id"),
                rs.getString("players.name"),
                rs.getInt("players.actions"),
    ...
    private GameCard readGameCard(ResultSet rs) throws Exception {
        return new GameCard(
                rs.getInt("gamecards.id"),
                this,
                rs.getString("gamecards.name"),
                rs.getString("gamecards.deck"),
                rs.getInt("gamecards.count"),
                rs.getInt("gamecards.cost"),
                rs.getInt("gamecards.value"),
                rs.getBoolean("gamecards.isaction"),
                rs.getBoolean("gamecards.iscoin")
        );
    }

    private PlayerCard readPlayerCard(ResultSet rs, Player player) throws Exception {
        return new PlayerCard(
                rs.getInt("playercards.id"),
                player,
                rs.getInt("playercards.card"),
                rs.getString("playercards.pile"),
                rs.getInt("playercards.order")
        );
    }
